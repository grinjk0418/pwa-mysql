...
FROM department_managers depm
	JOIN departments dept
		ON dept.dept_code = depm.dept_code
...
; -- SELECT에 쓰기 위해서 GROUP BY에 emp.`name`을 적는건 버그 위험 있어서 아래처럼 서브쿼리 사용한다.

-- 현재 각 부장별 이름, 연봉평균 (서브쿼리 사용한 방법)
SELECT
	emp.`name`
